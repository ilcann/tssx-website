...
import { getFooterLinks, footerContactItems } from "./footer.constants";
...
const Footer = () => {
  ...
  const footerLinks = getFooterLinks(lang); // 🌍 Dil bilgisine göre linkleri alıyoruz
      ...
      <section className="footer-content">
        <div className="footer-columns">
          <div className="footer-column lg:col-span-4">
            <Logo />
            <p className="footer-text">{t("footer.brand.description")}</p>
            <p className="footer-caption">{t("footer.brand.caption")}</p>
          ...
          <div className="footer-column lg:ml-6 lg:col-span-2">
            ...
            <ul className="footer-list">
              {footerLinks.companyLinks.map(({ href, labelKey }) => (
                <li key={href}>
                  <RouterLink to={href} className="footer-link">
                    {t(labelKey)}
                  </RouterLink>
                </li>
              ))}
                ...
                <li key={idx} className="flex flex-row gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    {lines.map((line, lineIdx) =>
                      typeof line === "string" ? (
                        <p key={lineIdx} className="footer-text">
                          {line}
                        </p>
                      ) : (
                        <a key={lineIdx} href={line.href} className="footer-link">
                          {line.text}
                        </a>
                      )
                    )}
